...
function alertContents() {
    if (http_request.readyState == 4) {
        if (http_request.status == 200) {
            ...
            // Devuelve un JSON
            var jsonDataRetrived = http_request.responseText;
            // Parseamos
            var canciones = JSON.parse(jsonDataRetrived);
            // Recorremos el arreglo
            var str = "";
            for (var i = 0; i < canciones.length; i++) {
                // Accedemos al atributo con .atributo
                str = str + '<li>' + canciones[i].titulo + '</li>';
            }
            // Añadimos al DOM
